...
__all__ = ["snli_module", "mnli_module", "anli_module"]
...
class ANLI:
    def __init__(self, options):
        ...
        self.train_snli, self.dev_snli, self.test_snli = MultiNLI.splits(
            self.TEXT, self.LABEL
        )
        self.train_mnli, self.dev_mnli, self.test_mnli = datasets.SNLI.splits(
            self.TEXT, self.LABEL
        )

        from torchtext.data import Dataset as Ds

        def combine_data(ds1, ds2):
            ds_concat = ds1 + ds2
            assert ds1.fields == ds2.fields
            list_of_ex = [x for x in ds_concat]
            new_ds = Ds(list_of_ex, ds1.fields)
            return new_ds

        train = combine_data(self.train_snli, self.train_mnli)
        dev = combine_data(self.dev_snli, self.dev_mnli)
        test = combine_data(self.test_snli, self.test_mnli)
...
class ANLIDataModule(pl.LightningDataModule):
    def __init__(self, conf):
        self.conf = conf
        self.batch_size = conf["batch_size"]

    def prepare_data(self):
        self.data = anli(self.conf)
...
def anli_module(conf):
    return ANLIDataModule(conf)
