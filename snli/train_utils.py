...
class SNLI_char_emb(SNLI_base):
    def __init__(self, model, conf, hparams, trial_set=None):
        super().__init__(model, conf, hparams, trial_set=trial_set)

    def forward(self, x0, x1, x0_char_vec, x1_char_vec):
        res = self.model.forward(
            x0, x1, x0_char_vec=x0_char_vec, x1_char_vec=x1_char_vec
        )
        return res

    def training_step(self, batch, batch_idx):
        x0, x0_char, x1, x1_char, y = batch
        opt = self.forward(x0, x1, x0_char, x1_char)
        opt = opt.squeeze(0)
        train_loss = F.cross_entropy(opt, y)
        result = pl.TrainResult(train_loss)
        result.log("training_loss", train_loss)
    ...
    def test_step(self, batch, batch_idx):
        x0, x0_char, x1, x1_char, y = batch
        opt = self(x0, x1, x0_char, x1_char)
        opt = opt.squeeze(0)
        test_loss = F.cross_entropy(opt, y)
        result = pl.EvalResult()
        metric = Accuracy(num_classes=3)
